...
long Minimizer::GradientDescent(tInterval A)
{
    if (I.isZero())
        throw myException("Error:- GradientDescentSearch: Zero interval for search.\n");
    if (!I.isInitialized())
        throw myException("Error:- GradientDescentSearch: Interval not initialized for search.\n");
    if (T == POLYNOMIAL && P.isZero()) throw myException("Error:- GradientDescentSearch: Polynomial not initialized.\n");
    if (T == FUNCTION && F.isZero()) throw myException("Error:- GradientDescentSearch: Function not initialized.\n");

    double x0= A.x, p= 0.01, x1,f1,d1;
    double tol= eps;
    long MAXITER= 1000;
    long COUNT = 0;
    x1 = x0;
    f1 = Fn(x1);
    d1 = - dF(x1);
    while(ABS(d1)>tol && COUNT<MAXITER)
    {
        x1 = x1 + p*d1;
        f1 = Fn(x1);
        d1 = - dF(x1);
