...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
try{
            Minimizer R(f[i],fInterval[i], GOLDEN_SECTION); // You need to change only the LINEAR
...
<<e.what() << endl;
    ...
    for (int i = 0; i < 3; i++) {
try{
            Minimizer R(P[i],fInterval[i], GOLDEN_SECTION); // You need to change only the LINEAR
                                                        // for other Minima finding functions
            long COUNT = R.findMinima();
            cout <<setw(5) << i + 1
<<setw(10) << COUNT
