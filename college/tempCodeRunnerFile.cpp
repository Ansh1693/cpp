...
int main()
{
 ...
 rbt[min_idx] = 0;
 prev_idx = -1;
 processes[min_idx][4] = currtime;                                      // CT
 ...
 isCompleted[min_idx] = 1;
    ...
    for (int i = 0; i < n; i++)
    {
        cout << "Process Id: " << processes[i][0] << " AT:  " << processes[i][1] << " BT: " << processes[i][2] << " ST: " << processes[i][3] << " CT: "
             << processes[i][4] << " RT: " << processes[i][5] << " WT: " << processes[i][6] << " TAT " << processes[i][7] << endl;
        sumtt += processes[i][7];
        sumwt += processes[i][6];
    }
